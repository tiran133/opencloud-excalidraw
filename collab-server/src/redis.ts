...
let _redisPub: Redis | Cluster | null = null
let _redisSub: Redis | Cluster | null = null
...
export type RedisMode =
  | { mode: 'cluster'; nodes: RedisNode[]; password?: string }
  | {
      mode: 'sentinel'
      sentinels: RedisNode[]
      name: string
 ...
 * Supports IPv4 (host:port), IPv6 bracket notation ([::1]:port), and bare IPv6.
 */
export function parseNodeList(raw: string, envName: string, defaultPort: number): RedisNode[] {
  const nodes = raw
    .split(',')
    ...
    .map((entry) => {
      ...
      if (!host) {
        throw new Error(`Invalid entry "${entry}" in ${envName}: host is missing`)
      }
...
/**
 * Determine which Redis mode to use based on environment variables.
...
export function resolveRedisMode(): RedisMode {
  ...
  if (!url) {
    throw new Error('REDIS_URL, REDIS_SENTINELS, or REDIS_CLUSTER_NODES must be set')
  }
...
export function isRedisConfigured(): boolean {
  return !!(process.env.REDIS_URL || process.env.REDIS_SENTINELS || process.env.REDIS_CLUSTER_NODES)
}

export function getRedisPub(): Redis | Cluster {
  if (!_redisPub) {
    _redisPub = createRedisClient()
    _redisPub.on('error', (err: Error) => console.error('[redis] pub client error:', err))
  }
  return _redisPub
}

export function getRedisSub(): Redis | Cluster {
  if (!_redisSub) {
    _redisSub = createRedisClient()
    _redisSub.on('error', (err: Error) => console.error('[redis] sub client error:', err))
  }
  return _redisSub
}
